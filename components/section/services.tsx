...
export function ServicesSection() {
  return (
    <section id="services" className="bg-[#0e0f0f] text-[#FEFCE1]">
      <div className="mx-auto max-w-[1365px] px-5 md:px-8 py-12 md:py-16 lg:pt-24 ">
        <h2 className="text-4xl md:text-6xl lg:text-[48px] text-[#FEFCE1]/85 font-normal tracking-tight">Services</h2>

        <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-5 md:gap-8 lg:gap-6 justify-items-stretch">
          {services.map((s, i) => {
            ...
            return (
              <article
                key={s.title}
                className={`group relative rounded-2xl bg-[#141513] border border-white/8 hover:border-white/15 transition-colors p-6 md:p-8 min-h-[200px] md:min-h-[220px] shadow-[0_0_0_1px_rgba(255,255,255,0.02)] ${spanClass}`}
              >
                ...
                <h3 className="mt-10 lg:mt-7 -ml-5 text-2xl md:text-3xl font-normal lg:text-4xl">{s.title}</h3>
                <p className="mt-2 md:mt-3 -ml-5 text-sm md:text-base leading-relaxed text-[#FEFCE1]/70">{s.desc}</p>
              </article>
            )
          })}
        </div>
